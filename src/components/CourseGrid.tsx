...
  const createScrollHandlers = (scrollRef, setCurrentPage) => {
    const handleScroll = () => {
...
  const mujHandlers = createScrollHandlers(mujScrollRef, setCurrentMujPage);
  const smuHandlers = createScrollHandlers(smuScrollRef, setCurrentSmuPage);

  useEffect(() => {
    if (mujScrollRef.current) {
      mujScrollRef.current.addEventListener('scroll', mujHandlers.handleScroll);
      return () => {
        mujScrollRef.current?.removeEventListener('scroll', mujHandlers.handleScroll);
      };
    }
  }, [mujHandlers]);

  useEffect(() => {
    if (smuScrollRef.current) {
      smuScrollRef.current.addEventListener('scroll', smuHandlers.handleScroll);
      return () => {
        smuScrollRef.current?.removeEventListener('scroll', smuHandlers.handleScroll);
      };
    }
  }, [smuHandlers]);
...
                    index === currentMujPage ? "bg-primary w-4 h-4" : "bg-gray-400"
                  }`}
                />
              ))}
            </div>
...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    index === currentSmuPage ? "bg-primary w-4 h-4" : "bg-gray-400"
                  }`}
                />
              ))}
            </div>
